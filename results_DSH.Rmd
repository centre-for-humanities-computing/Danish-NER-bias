...
collapse_rows_df(both_metrics_tb1, Model) %>% 
  kbl(
    ...
    caption = "Table 1: NER Performance (Average Precision and Recall Excl. MISC)",
    #format="latex",
  ...
  collapse_rows() %>% 
  add_footnote("variables: ents_excl_MISC_ents_p and ents_excl_MISC_ents_r") %>% 
...
collapse_rows_df(both_metrics_tb2, Model) %>% 
  kbl(
    booktabs=T, 
    col.names = c("Model", "Metric", "Majority", "Minority", "Majority", "Minority", "Majority", "Minority", "Majority"),
    caption = "Table 2: NER scores for 'PER' Entity (Average Precision and Recall)",
    #format="latex",
    align=c("l", rep("c", nrow(.)-1)),
    table.attr = "style='width:30%;'"
    ) %>% 
  add_header_above(c(" " = 1, " " = 1, "All" = 2, "Men" = 2, "Women" = 2, "Unisex"=1)) %>% 
  collapse_rows() %>% 
  add_footnote("variables: ents_per_type_PER_p and ents_per_type_PER_r") %>% 
  kable_classic(full_width = F, html_font = "Cambria") 
```
